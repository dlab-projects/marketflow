...
# SECSTAT == "R" indicates that the security is "Regular" (i.e. the security is
# past the "When-Issued" stage and the company is not going through bankruptcy
# proceedings)
dsenames = dsenames[dsenames['SECSTAT'] == "R"]
# TRDSTAT == "A" indicates that the security is actively trading
dsenames = dsenames[dsenames['TRDSTAT'] == "A"]
...
# There are 239 cases where the length of tsymbol does not match up with the
# length of ticker For most of these cases, it is because tsymbol includes a
# suffix of some sort, while ticker does not
...
# The 'S' suffix indicates the "Shares of Beneficial Interest, which do not
# trade like regular securities The 'V' and 'WI' suffixes indicate
# "When-Issued" shares, which have been authorized to trade, but have not
# actually begun trading
nasdaq = dsenames.PRIMEXCH == "Q"
beneficial = (dsenames['LAST'] == "S") & (dsenames['SYMBOL_LEN'] == 5)
whenissued_nasdaq = ((dsenames['LAST'] == "V") &
                     (dsenames['SYMBOL_LEN'] == 5) & nasdaq)
whenissued_nonnasdaq = ((dsenames['LAST2'] == "WI") &
                        (dsenames['SYMBOL_LEN'] > 3) & ~nasdaq)
...
# For non-NASDAQ exchanges (e.g. the New York Stock Exchange), ticker symbols
# without suffixes are 3 characters or less.
flag4 = ~nasdaq & (dsenames.SYMBOL_LEN > 3) & class_equal_last

# There is a fifth set of suffixed ticker symbols that do not fit into the
# above categories, but they do have a unifying manual adjustment. We denote
# this set as "funny" (not "funny" ha ha).

funny_permnos = [85254, 29938, 29946, 93093, 92118, 83275, 82924, 82932, 77158,
                 46950, 90655]
...
# Here, we group together the symboled suffixes to make the final root-suffix
# separation cleaner. `sym5_with_suffix` is the set of special cases with more
# than 4 characters in the symbol
sym5_with_suffix = flag1 | flag2 | flag3
symbol_with_suffix = flag4 | funny | sym5_with_suffix

# Finally, the big enchilada, the separation of each ticker symbol into its
# root and its symbol. Since we are only dealing with suffixes of length 1, the
# root will consist of all but the last character, and the root will be the
# ticker symbol's last character
...
# Finally, we want to ensure that, when the same information is recorded, the
# date range listed for the record reflects the entire range over which the
# security was actively trading.

# For instance, if a security stopped trading for a six month period, it has
# two entries in this file. We want both of those entries to include beginning
# date for the security's trading before the six month break and the end date
# for the security's trading after the six month break.

# To do this, we first want to reset the index in the dsenames dataframe
dsenames = dsenames.reset_index(drop=True)

# When we say that we want to adjust the dates 'when the same information is
# recorded,' we make that adjustment based on the following seven variables in
# the data frame:
# - Permno, the two components of the ticker symbol, the name of the company
# the CUSIP number (current and historical), and
# - the primary exchange on which the security trades

# We first create a new data frame sorted on these 7 columns, which only
# includes said 7 columns

levels_sort = ["PERMNO", 'SYM_ROOT', 'SYM_SUFFIX', 'COMNAM', 'CUSIP', 'NCUSIP',
               'PRIMEXCH']
dsenames_sort = dsenames.sort_values(by=levels_sort).loc[:, levels_sort]
dsenames = dsenames.sort_values(by=levels_sort)

# We create two new variables, begdate and enddate, to capture the full range
# of dates for which each security trades. The default case, when a security
# only matches with itself based on the 7 sort levels, is that the beginning
# date is the same as the beginning effective name date, and the end date is
# the same as the end effective name date.
...
# We then create a boolean matrix based on whether the entries of each matrix
# match
compare_matrix = (dsenames_nofirst == dsenames_nolast)

# If the i-th record matches the next record for all 7 variables, then the i-th
# row of the compare matrix will be all true. We extract the index for
# subsetting purposes
same_as_below = compare_matrix.all(axis=1)
same_as_below_index = same_as_below.index[same_as_below]

# In order to collapse the end dates, we will also need an index to indicate if
# a record is the same as the one above.  This is simply caputured by adding 1
# to the first index we found
same_as_above_index = same_as_below_index + 1

# Finally, we loop through the first Int64Index we found to bring the earliest
# `BEGDATE` for a record down to all of its matches.  Doing this matching
# iteratively mitigates the issue of a particular security having more than 2
# records match based on the 7 variables.
...
# Finally, we output a final dataframe that includes only the columns we sorted
# on and our new date variables.  Since the same information is recorded for
# these files now, we drop the duplicates
final_columns = levels_sort + ['BEGDATE', 'ENDDATE']

clean_dsenames = \
    dsenames.drop_duplicates(subset=final_columns).loc[:, final_columns]
